...
const GUEST_CART_KEY = "indian_guest_cart";
...
export const addToGuestCart = async (productVariantId, quantity = 1) => {
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4004/api" : "https://api.indianpharmazee.com/api")}/public/products/variants/${productVariantId}`,
            {
                credentials: "include",
            }
        );
        ...
        const mergePromises = guestItemsToMerge.map(async (guestItem) => {
            try {
                ...
                const addResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4004/api" : "https://api.indianpharmazee.com/api")}/cart/add`,
                    {
                        method: "POST",
                        headers: {
