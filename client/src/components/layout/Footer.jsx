...
import {
  ...
  ArrowUpRight, Thermometer, ShieldCheck, Truck, BadgeCheck, Clock,
} from "lucide-react";
...
export const Footer = () => {
  ...
  return (
    <footer className="relative overflow-hidden" style={{ background: "#e2e8f0" }}>
      ...
      {/* ── Decorative glows ── */}
      <div className="absolute top-20 left-0 w-72 h-72 rounded-full blur-[120px] opacity-5 pointer-events-none" style={{ background: "#0074e4" }} />
      <div className="absolute bottom-20 right-0 w-60 h-60 rounded-full blur-[100px] opacity-5 pointer-events-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4">
            ...
            <ul className="space-y-3 mb-7">
              <li>
                <a href="mailto:indianpharmazee@gmail.com" className="group flex items-center gap-3 text-sm text-slate-600 hover:text-blue-600 transition-colors">
                  <span className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors group-hover:bg-blue-600/10" style={{ background: "rgba(0,94,184,0.08)" }}>
              ...
              <li>
                <a href="tel:+919560247619" className="group flex items-center gap-3 text-sm text-slate-600 hover:text-blue-600 transition-colors">
                  <span className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors group-hover:bg-blue-600/10" style={{ background: "rgba(0,94,184,0.08)" }}>
              ...
              <li>
                <a
                  ...
                  className="group flex items-center gap-3 text-sm text-green-600 hover:text-green-700 transition-colors"
                >
              ...
              <li className="flex items-center gap-3 text-sm text-slate-600">
                <span className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(0,94,184,0.08)" }}>
            ...
            <div className="flex gap-2.5">
              {SOCIALS.map((s) => (
                <a
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(0,94,184,0.1)"; e.currentTarget.style.borderColor = "rgba(0,94,184,0.2)"; }}
            ...
            <ul className="space-y-2.5">
              {(categories.length > 0 ? categories.map(c => ({ label: c.name, href: `/category/${c.slug}` })) : SPECIALTIES.map(s => ({ label: s, href: "/products" }))).map((item) => (
                <li key={item.label}>
                  ...
                  >
                    <span className="w-1 h-1 rounded-full flex-shrink-0 group-hover:scale-150 transition-transform" style={{ background: "#0074e4" }} />
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-2.5">
              {QUICK_LINKS.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="group flex items-center gap-2 text-sm text-slate-600 hover:text-blue-600 transition-colors">
                    <span className="w-1 h-1 rounded-full flex-shrink-0 group-hover:scale-150 transition-transform" style={{ background: "#0074e4" }} />
                    {l.label}
                  </Link>
                </li>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {["UPI", "Visa", "Mastercard", "Net Banking", "Razorpay"].map((m) => (
                <span
                  key={m}
                  className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-slate-600"
                  style={{ background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.06)" }}
                >
                  {m}
                </span>
            ...
            <div
              ...
              style={{ background: "rgba(0,94,184,0.04)", borderColor: "rgba(0,94,184,0.12)" }}
            >
      ...
      {/* ── Divider ── */}
      <div style={{ height: "1px", background: "linear-gradient(90deg, transparent, rgba(0,0,0,0.05) 30%, rgba(0,94,184,0.1) 50%, rgba(0,0,0,0.05) 70%, transparent)" }} />

      {/* ── Bottom bar ── */}
      <div className="max-w-7xl mx-auto px-6 py-5 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-500">
            © {new Date().getFullYear()} <span className="text-slate-600 font-medium">Indian Pharmazee</span>. All rights reserved.
          </p>
          <p className="text-xs text-slate-500 text-center">
            Prescription medicines require a valid prescription. Always consult a qualified physician.
          </p>
          <div className="flex items-center gap-4">
            <Link href="/privacy-policy" className="text-xs text-slate-500 hover:text-slate-900 transition-colors">Privacy</Link>
